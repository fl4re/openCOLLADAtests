...
class FRenderer:
	...
	def DoRender(self, input_filename): 
		...
		export = subprocess.Popen( self.configDict["renderExe"] + ' ' + '-rd' + ' ' + self.outputDir + ' ' + cmd + ' ' + '-im' + ' ' + self.imageName + ' ' + input_filename, stdout = log, stderr = subprocess.STDOUT)
	#	export = subprocess.Popen('C:\\Program Files\\Autodesk\\Maya2015\\bin\\Render -rd "F:\Dev\COLLADA-CTS\_Test" -cam "|testCamera" -r ctfHw -x 512 -y 512 -ard 1.0 -s 1 -e 1 -b 1 -of png  -im "foot" "F:\Dev\COLLADA-CTS\_Test\Cube.mb"', stdout = log, stderr = subprocess.STDOUT)
